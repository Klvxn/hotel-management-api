...
@room_router.get("/as_admin", response_model=list[Room_Pydantic])
async def admin_get_rooms(
    ...
    filters = {"booked": booked, "room_type": room_type}
    query = filter_room_query(Room.all(), filters).prefetch_related("reservations")
...
@room_router.get("/{room_id}/as_admin", response_model=Room_Pydantic)
...
async def get_single_reservation(
    reservation_id: UUID, current_user: BaseUser = Depends(get_current_active_user)
):
...
async def delete_review(
    review_id: int, current_user: BaseUser = Depends(get_current_active_user)
):
    review_obj = await Review.get(id=review_id).select_related("customer")
    if not current_user.is_admin:
        await authorize_obj_access(review_obj, current_user)
